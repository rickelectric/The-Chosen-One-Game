...
public class ImageSFXs {
	...
	private static ImageSFXs thisInst = null;
	...
	public static synchronized ImageSFXs getInstance() {
		if (thisInst == null) {
			thisInst = new ImageSFXs();
		}
		return thisInst;
	}
	...
	/*
	 * 
	 * private void initEffects() // Create pre-defined image operations for
	 * image negation and blurring. { // image negative. Multiply each colour
	 * value by -1.0 and add 255 negOp = new RescaleOp(-1.0f, 255f, null);
	 * 
	 * // image negative for images with transparency float[] negFactors =
	 * {-1.0f, -1.0f, -1.0f, 1.0f}; // don't change the alpha float[] offsets =
	 * {255f, 255f, 255f, 0.0f}; negOpTrans = new RescaleOp(negFactors, offsets,
	 * null);
	 * 
	 * 
	 * // blur by convolving the image with a matrix float ninth = 1.0f / 9.0f;
	 * 
	 * float[] blurKernel = { // the 'hello world' of Image Ops :) ninth, ninth,
	...
	public BufferedImage getRotatedImage(BufferedImage src, int directionAngle)
	/*
	 ...
	 */
	{
		...
		BufferedImage dest = gc.createCompatibleImage(src.getWidth(),
		...
		rot.rotate(Math.toRadians(directionAngle), src.getWidth() / 2,
				src.getHeight() / 2);
		g2d.transform(rot);
	...
	// make a copy of the BufferedImage src
	{
		...
		int transparency = src.getColorModel().getTransparency();
		BufferedImage copy = gc.createCompatibleImage(src.getWidth(),
				src.getHeight(), transparency);
		Graphics2D g2d = copy.createGraphics();
	 ...
	 */
	{
		...
		System.out.print("In ImageSFXs: width = " + src.getWidth());
